...
def fake_raw_ldap_data(nbr_users=10):
    ...
    return list([
    {
        ...
        "homeDirectory": [
            f"/home/john.smith{i:03d}"
        ],
        "loginShell": [
            "/bin/bash"
        ],
        "mail": [
            f"john.smith{i:03d}@mila.quebec"
        ],
        "memberOf": [],
        "objectClass": [
            "top",
...
def test_query_to_ldap_server_and_writing_to_output_json(monkeypatch):
    ...
    def mock_query_ldap(local_private_key_file, local_certificate_file, ldap_service_uri):
...
def test_query_to_ldap_server_and_commit_to_db(monkeypatch):
    ...
    for u1, u2 in zip(sorted(L_users, key=sorted_order_func),
    ...
    for u1, u2 in zip(sorted(L_users, key=sorted_order_func),
                      sorted(transform_user_list(L_first_batch_users+L_second_batch_users), key=sorted_order_func)):
        assert u1 == u2
