...
def test_config_read_write_into_db(empty_read_write_db, beans_config):
    ...
    assert db_state.check == state.check
    assert db_state.last_result == state.last_result
    assert db_state.last_message == state.last_message
...
def test_get_states_returns_sorted(empty_read_write_db):
    for name in ["charlie", "alpha", "bravo"]:
        ...
        HealthCheckStateDB.get_or_create(
            empty_read_write_db, HealthCheckState(check=hc)
        )
...
def test_HealthCheckStateRepository(empty_read_write_db, tmpdir):
    ...
    retrieved = HealthCheckStateDB.get_state(empty_read_write_db, "test_check")
    ...
    retrieved_updated = HealthCheckStateDB.get_state(empty_read_write_db, "test_check")
    assert retrieved_updated.id == retrieved.id
