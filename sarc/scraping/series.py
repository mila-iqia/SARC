...
def get_job_time_series_batched(
    ...
    # Map job_id strings to list of original job indices to group results
    job_map: dict[str, list[int]] = defaultdict(list)
    results: list[list] = [[] for _ in jobs]

    # 2. Build precise subqueries for each valid job
    for idx, job in enumerate(jobs):
        ...
        # Duration should not be looking into the future
        if offset < 0:
        ...
        label_exprs.append(f'slurmjobid="{str_job_id}"')
        ...
        if measure and aggregation:
            ...
            q = f"{selector}[{range_seconds}s]"
            if "(" in measure:
                q = measure.format(f"{q} {offset_string}")
            else:
                q = f"{measure}({q} {offset_string})"
            q = f"{q}[{duration_seconds}s:{range_seconds}s]"
        else:
            q = f"{selector}[{duration_seconds}s:{interval}s] {offset_string}"

        # Parenthesize subqueries for operator safety
        subqueries.append(f"({q})")
        job_map[str_job_id].append(idx)

    # If no jobs in the batch are queryable, return empty results for all
    if not subqueries:
        return results

    # 3. Execute batched PromQL query in 1 HTTP call
    combined_query = " or ".join(subqueries)
    ...
    # 4. Map returned series back to input jobs by position
    for series_data in raw_results:
        job_id = series_data.get("metric", {}).get("slurmjobid")
        if not job_id or job_id not in job_map:
            continue

        matching_indices = job_map[job_id]

        if len(matching_indices) == 1:
            # Single unique job ID in batch
            results[matching_indices[0]].append(series_data)
        else:
            # Preemption/requeue edge case: Multiple jobs in this batch share the same job_id.
            # Disambiguate series data by checking timestamp bounds for each job run.
