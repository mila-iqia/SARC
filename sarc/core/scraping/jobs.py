...
from sarc.core.scraping.jobs_utils import DATE_FORMAT_HOUR, parse_auto_intervals, parse_intervals, set_auto_end_time, SacctScraper, update_allocated_gpu_type_from_nodes
...
def fetch_jobs(cluster_names: list[str], clusters: dict[str, ClusterConfig], unparsed_intervals: Optional[list[tuple[datetime, datetime]]], auto_interval: Optional[int]) -> None:
    """
...
def parse_jobs(cluster_names: list[str], clusters: dict[str, ClusterConfig],  from_: datetime) -> None: #Iterable[SlurmJob]:
