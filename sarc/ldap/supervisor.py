...
mapping_prof_mila_email_to_academic_email = load_python_dict(
    get_filename("mapping_prof_mila_email_to_academic_email.py")
)
...
def _student_or_prof(person, exceptions):
    ...
    if person["mail"][0] in exceptions["not_student"]:
    ...
    elif person["mail"][0] in exceptions["not_teacher"]:
    ...
    if is_student:
        ...
        if len(L) == 1:
            ...
            raise MultipleSupervisor(f"More than two supervisors for a student: {L}.")
...
def filter_raw_ldap(json_data, exceptions):
    L_profs = []
    L_students = []

    for person in json_data:
        prof, student = _student_or_prof(
            person,
            exceptions,
        )

        person["enriched"] = dict(student=student, prof=prof)
...
def find_and_match_supervisors(population_mila_csv_input_path, ldap_data):
    L_profs, L_students = filter_raw_ldap(ldap_data, dict(
        not_student=not_sudents,
        not_teacher=not_prof,
        rename=rename,
    ))

    # Suplemental data provided by the Academic office
    if population_mila_csv_input_path:
...
def run(population_mila_csv_input_path, ldap_data, verbose=True):
    L_profs, L_students = find_and_match_supervisors(
        population_mila_csv_input_path, ldap_data
    )
...
if __name__ == "__main__":
    with open(get_filename("mila_raw_users.json")) as file:
        ldap_data = json.load(file)

        find_and_match_supervisors(
